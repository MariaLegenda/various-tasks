...
def print_top5(data):
    ...
    # print('{: <35} | {} | {: >7.2f} | {: >5} | {: >6.1f} | {: >6.1f} |'.format(
        ...
        print(f'{row[0]: <35} | {row[1]} | {row[2]: >7.2f} | {row[3]: >5} | {row[5]: >6.1f} | {row[6]: >6.1f} |')
...
# окупаемость. ROI = (СБОРЫ – БЮДЖЕТ) / БЮДЖЕТ
def add_roi(data):
...
# стоимость 1 минуты фильма: 
def add_price_per_minute(data):
    for i in range(len(data)):
        min_cost = data[i][5] / data[i][3]
        data[i].append(min_cost)
...
print('Название                            | ROI  | Цена за минуту, млн $ |')
print('-' * 68)
for row in oscar_data[:5]:
    print(f'{row[0]: <35} | {row[7]: >4.1f} | {row[8]: >21.2f} |')
...
print('\nФильмы не длинее 120 минут:')
new_data = filter_not_longer_than(oscar_data, 120)
print_top5(new_data)


# фильтрация таблицы по жанрам
...
for row in genres_means:
    print(f'{row[0]: <9} | {row[1]: >7.2f} | {row[2]: >5.2f} | {row[3]: >5.2f} | {row[4]: >16.2f}')


# исключение фильмов заданного жанра
...
print('\nФильмы с 2000 по 2010 год:')
filtered_data = filter_by_year(oscar_data, 2000, 2010)
print_table(filtered_data)

# Поделим 30 лет на 3 периода по 10 лет. Для фильмов из каждого диапазона 
# посчитаем средние рейтинг, длину, стоимость съёмок одной минуты и сборы.
...
for begin_end in years:
    ...
    new_data = filter_by_year(oscar_data, begin, end)
    
    mean_score = column_mean(new_data, 2)
    mean_len = column_mean(new_data, 3)
    mean_ppm = column_mean(new_data, 7)
    mean_gross = column_mean(new_data, 6)
    
    rows.append([name, mean_score, mean_len, mean_ppm, mean_gross])

print('\nСтатистика по десятилеткам:')   
print('Годы      | Рейтинг | Длина  | Бюджет за минуту | Сборы ')
print('--------------------------------------------------------')
for row in rows:
    print(f'{row[0]: <9} | {row[1]: >7.2f} | {row[2]: >5.2f} | {row[3]: >16.2f} | {row[4]: >6.2f}')
